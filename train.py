...
if __name__ == '__main__':
    ...
    net = ActorCritic(input_dim=env.state_dims,
                      output_dim=env.action_dims).to(device)
    if len(glob.glob(os.path.join(ckpt_folder, '*.pt'))) > 0:
        # load the last ckpt
        checkpoint = torch.load(
            glob.glob(os.path.join(ckpt_folder, '*.pt'))[-1])
    ...
    for episode_id in range(last_episode_id, max_m_episode):
        ...
        for step_id in range(max_steps):
            ...
            if done or step_id == max_steps-1:
                ...
                net.update_ac(net, rewards, log_probs, values,
                              masks, Qval, gamma=0.999)
                break
        ...
        if episode_id % 100 == 1:
            ...
            plt.ylabel('reward')
            plt.savefig(os.path.join(ckpt_folder, 'rewards_' +
                        str(episode_id).zfill(8) + '.jpg'))
            plt.close()
